...
TARFILE=Linux_RR_${vPDBName}.tar
vObjectCountOut="object_count_aix_${vPDBName}.out"
vRowCountOut="row_count_aix_${vPDBName}.out"
...
vDBLinks=$($ORACLE_HOME/bin/sqlplus -S "system/${vSystemPwd}@$vPDBName" << RUNSQL
set pagesize 0 linesize 32767 feedback off verify off heading off echo off
SELECT distinct db_link FROM dba_db_links where host not like '%acxiom%';
EXIT;
RUNSQL
)
...
do
	...
	vLinkOwner=$($ORACLE_HOME/bin/sqlplus -S "system/${vSystemPwd}@$vPDBName" << RUNSQL
set pagesize 0 linesize 32767 feedback off verify off heading off echo off
...
RUNSQL
	...
	do
		...
		if [[ $ownername = "PUBLIC" ]]
		then
			vLinkTest=$($ORACLE_HOME/bin/sqlplus -S "system/${vSystemPwd}@$vPDBName" << RUNSQL
set pagesize 0 linesize 32767 feedback off verify off heading off echo off
...
RUNSQL
			...
			then
				...
				# check if HS link
				vHSLinkCheck=$($ORACLE_HOME/bin/sqlplus -S "system/${vSystemPwd}@$vPDBName" << RUNSQL
set pagesize 0 linesize 32767 feedback off verify off heading off echo off
...
connect system[${ownername}]/${vSystemPwd}@$vPDBName
select '1' from dual@${linkname};
connect system/${vSystemPwd}@$vPDBName
...
# exit script if failed links are found
if [[ $vFailedLinkCt -gt 0 ]]
then
	echo "" | tee -a $vOutputLog
	echo "ERROR: All DB links must be fixed before continuing." | tee -a $vOutputLog
	echo "       $vFailedLinkCt link(s) are not working." | tee -a $vOutputLog
	exit 1
else
	echo "" | tee -a $vOutputLog
	echo "All database links are working" | tee -a $vOutputLog
fi
...
if [[ $vDBVersion -eq 12 ]]
then
	$ORACLE_HOME/bin/sqlplus -s sys/$vSysPwd@$vPDBName as sysdba << RUNSQL
SET ECHO ON
SET DEFINE ON
SET ESCAPE OFF
SET FEEDBACK ON
