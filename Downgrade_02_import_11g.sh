...
#================================================================================================#
#  NAME
#    Downgrade_02_Linux_build.sh
#
#  DESCRIPTION
#    Create a new 11g database, import data from 12c database
#
#  NOTES
#    Run as oracle
...
vErrPumpIgnore+=(ORA-39111)     # Dependent object type PASSWORD_HISTORY:"XXX" skipped, base object type USER:"XXX" already exists
...
RUNSQL
	...
	then
		...
		echo "" | tee -a $vOutputLog
		echo "This script can only be used for 11g AL32UTF8 and US7ASCII databases. " | tee -a $vOutputLog
		exit 1
	fi
...
function aix_files_fnc {
	...
	if [[ ! -e $TARFILE ]]
	then
		TARFILE="${vDBScripts}/Linux_downgrade_${vPDBName}.tar"
		if [[ ! -e $TARFILE ]]
		then
			echo " " | tee -a $vOutputLog
	...
	rm *.oby
	rm *.out
	tar -xf Linux_downgrade_${vPDBName}.tar
	if [ $? -ne 0 ]
	then
		echo " " | tee -a $vOutputLog
...
function variables_fnc {
	...
	TARFILE="/tmp/Linux_downgrade_${vPDBName}.tar"
...
insert into $vLinuxProfilesTable values ('APP_PROFILE','LOGICAL_READS_PER_CALL','DEFAULT','YES');
...
insert into $vLinuxProfilesTable values ('CNO_PROFILE','LOGICAL_READS_PER_CALL','DEFAULT','YES');
...
insert into $vLinuxProfilesTable values ('APP_PROFILE','LOGICAL_READS_PER_CALL','DEFAULT');
...
insert into $vLinuxProfilesTable values ('CNO_PROFILE','LOGICAL_READS_PER_CALL','DEFAULT');
