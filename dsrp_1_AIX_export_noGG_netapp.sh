...
. ${RUNDIR}/dsrp_1_AIX_export.param
...
vDPDataLog="expdp_${ORACLE_SID}.log"
vDPDataDump="${ORACLE_SID}_%U.dmp"
vDPImpPar="${vOutputDir}/impdp_${DBNAME}.par"

vDPDataParCC="${vDPParDir}/expdp_${DBNAME}_cure_core.par"
vDPDataLogCC="expdp_${ORACLE_SID}_cure_core.log"
vDPDataDumpCC="${ORACLE_SID}_cure_core_%U.dmp"
vDPImpParCC="${vOutputDir}/impdp_${DBNAME}_cure_core.par"

vDPDataParCQ="${vDPParDir}/expdp_${DBNAME}_cure_queue.par"
vDPDataLogCQ="expdp_${ORACLE_SID}_cure_queue.log"
vDPDataDumpCQ="${ORACLE_SID}_cure_queue_%U.dmp"
vDPImpParCQ="${vOutputDir}/impdp_${DBNAME}_cure_queue.par"

vDPMetaPar="${vDPParDir}/expdp_metadata_${DBNAME}.par"
vDPMetaLog="expdp_${ORACLE_SID}_metadata.log"
vDPDumpMeta="${ORACLE_SID}_metadata_%U.dmp"

echo "" | tee -a $vOutputLog
echo "Removing existing logs" | tee -a $vOutputLog
for file in $SETPARAM $vOrigParams $CREATETS $REDOLOGS $UNDOSIZE $TEMPSIZE $TSGROUPS $SYSOBJECTS $vCreateUsers $vCreateCommonUsers $CREATEQUOTAS $CREATEGRANTS $CREATESYSPRIVS $REVOKESYSPRIVS $CREATESYNS $CREATEROLES $CREATELOGON $CREATETRIGGERS $vCreateExtTables $vDefaultDates $vCreateDBLinks $vRefreshGroups $vProxyPrivs /tmp/${TARFILE} $vDPDataPar $vDPMetaPar $vDPImpPar $SCNFILE ${vDPParDir}/${vDPDataLog} ${vDPParDir}/${vDPMetaLog} $vCreateACL $vLockUsers $vUnlockUsers $vCheckUsers $vAccountStatus
do
	if [[ -e $file ]]
	then
...
and ob.object_type='INDEX' and ob.generated='N' and ix.owner not in ($vExcludeUsers);
...
DECLARE
  CURSOR c1 IS
	...
	from dba_users 
	where username not in ('CURE_CORE','CURE_QUEUE')
	--where username not in ('GGTEST','JIRA')
	and username not in ($vExcludeUsers);
  vOwner	VARCHAR2(30);
  x			PLS_INTEGER;
BEGIN
...
error_check_fnc $vOutputLog $vErrorLog $vCritLog
for file in $SETPARAM $vOrigParams $REDOLOGS $CREATETS $TEMPSIZE $TSGROUPS $SYSOBJECTS $CREATEQUOTAS $CREATEROLES $CREATEGRANTS $CREATESYSPRIVS $REVOKESYSPRIVS $vProxyPrivs $CREATESYNS $CREATELOGON $vCreateExtTables $vDPDataPar $vDPMetaPar
do
	if [[ -e $file ]]
	then
...
BEGIN
  ...
  DBMS_OUTPUT.PUT_LINE('METRICS=Y');
  DBMS_OUTPUT.PUT_LINE('TRANSFORM=TABLE_COMPRESSION_CLAUSE:"COMPRESS FOR OLTP"');
...
echo "DIRECTORY=CNO_MIGRATE" > $vDPDataParCC
echo "DUMPFILE=${vDPDataDumpCC}" >> $vDPDataParCC
echo "LOGFILE=${vDPDataLogCC}" >> $vDPDataParCC
echo "PARALLEL=${vParallelLevel}" >> $vDPDataParCC
echo "METRICS=Y" >> $vDPDataParCC
echo "EXCLUDE=STATISTICS" >> $vDPDataParCC
#echo "SCHEMAS=JIRA" >> $vDPDataParCC
echo "SCHEMAS=CURE_CORE" >> $vDPDataParCC

echo "DIRECTORY=CNO_MIGRATE" > $vDPDataParCQ
echo "DUMPFILE=${vDPDataDumpCQ}" >> $vDPDataParCQ
echo "LOGFILE=${vDPDataLogCQ}" >> $vDPDataParCQ
echo "PARALLEL=${vParallelLevel}" >> $vDPDataParCQ
echo "METRICS=Y" >> $vDPDataParCQ
echo "EXCLUDE=STATISTICS" >> $vDPDataParCQ
#echo "SCHEMAS=GGTEST" >> $vDPDataParCQ
echo "SCHEMAS=CURE_QUEUE" >> $vDPDataParCQ
...
# export CURE_CORE schema
expdp \"/ as sysdba\" flashback_scn=${CURRENT_SCN} parfile=$vDPDataParCC
cat ${vDPParDir}/${vDPDataLogCC} >> $vOutputLog
error_check_fnc ${vDPParDir}/${vDPDataLogCC} $vErrorLog $vCritLog

# export CURE_QUEUE schema
expdp \"/ as sysdba\" flashback_scn=${CURRENT_SCN} parfile=$vDPDataParCQ
cat ${vDPParDir}/${vDPDataLogCQ} >> $vOutputLog
error_check_fnc ${vDPParDir}/${vDPDataLogCQ} $vErrorLog $vCritLog
...
echo "scp ${vOutputDir}/${TARFILE} oracle@${NEWHOST}:${vNewDir}" | tee -a $vOutputLog
